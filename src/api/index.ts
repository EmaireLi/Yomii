...
function normalizeEssay(data: any): Essay {
  return {
    ...
    progressMessage: String(data?.progressMessage ?? data?.progress_message ?? ''),
  ...
  return {
    ...
    job: data?.job
      ? {
          ...
          progressPercent: Number(data.job.progressPercent ?? data.job.progress_percent ?? 0),
          progressMessage: String(data.job.progressMessage ?? data.job.progress_message ?? ''),
