...
from common import format_revision_prompt, load_jsonl_dataset, load_yaml_config


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train revision LoRA for JLPT essay revision.")
    parser.add_argument(
        "--config",
        default=str(Path(__file__).parent / "configs" / "revision_lora.yaml"),
...
def main() -> int:
    ...
    train_dataset = load_jsonl_dataset(cfg["train_file"]).map(
        lambda row: tokenizer(format_revision_prompt(row), truncation=True, max_length=cfg["max_seq_length"]),
